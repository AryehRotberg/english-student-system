...
1. Trigger generation with `POST /ai-contents/generate-quiz` using `topic`, `multipleChoiceCount`, `openEndedCount`, and optionally `additionalInstructions`.
...
3. The audio stream is buffered server-side and returned directly as `audio/mpeg` at 44100 Hz / 128 kbps.

  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      if (!isUuid(payload.attemptId)) {
        throw new Error('Quiz attempt ID is missing or invalid. Set a valid UUID before submitting answers.');
      }
    ...
    onSuccess: async (_data, variables) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['student-answers', variables.attemptId] });
    },
  });
}
...
export function useCreateQuestion() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: { question: string; questionType: string; audioUrl?: string }) =>
...
export function useCreateQuestionOption() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: { questionId: string; optionText: string; isCorrect: boolean }) =>
...
export function useUpdateQuestionOption() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, questionId, ...payload }: { id: string; questionId: string; optionText?: string; isCorrect?: boolean }) =>
...
export function useCreateAnswer() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: { questionId: string; answer: string; blankIndex: number }) =>
...
export function useUpdateAnswer() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...payload }: { id: string; questionId?: string; answer?: string; blankIndex?: number }) =>
...
export function useCreateQuizQuestion() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: { quizId: string; questionId: string; maxPoints: number }) =>
      quizQuestionsService.create(payload),
    onSuccess: (_data, variables) =>
      queryClient.invalidateQueries({ queryKey: ['raw-quiz-questions', variables.quizId] }),
  });
}
...
export function useUpdateQuizQuestion() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...payload }: { id: string; quizId?: string; questionId?: string; maxPoints?: number }) =>
...
export function useCreateText() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: { title: string; content: string; level: string }) =>
...
export function useLogin() {
  ...
  return useMutation({
    mutationFn: async (payload: { email: string; password: string }) => authService.login(payload),
    onSuccess: async () => {

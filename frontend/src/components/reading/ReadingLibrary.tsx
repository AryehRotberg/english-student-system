  ...
  return (
    <section className={styles.panel}>
      ...
      <div className={styles.filters}>
        ...
        {levels.map((level) => (
          <button
            className={level === selectedLevel ? `${styles.chip} ${styles.chipActive}` : styles.chip}
            key={level}
          ...
          <article key={item.id} className={styles.row}>
            <div>
              <h3 className={styles.title}>
                {item.title} <span className={styles.level}>({item.level})</span>
              </h3>
              <p className={styles.meta}>{item.minutes} minute read</p>
            </div>

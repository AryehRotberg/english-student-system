...
export function TaskGrid({ tasks, assignmentTopics, onOpenQuiz }: TaskGridProps) {
  return (
        ...
        {assignmentTopics.length > 0 ? (
          assignmentTopics.map((topic) => (
            <button
              ...
              type="button"
            >
              <span className={styles.topicAssignment}>{topic.assignmentTitle}</span>
              <span className={styles.topicDescription}>{topic.assignmentDescription}</span>

...
export class AssignmentItemsService {
    ...
    async findByUserId(filter: GetAssignmentItemsFilterDto): Promise<AssignmentItemResponseDto[]> {
        const { userId } = filter;

        const assignmentItems = await this.postgresService.query<AssignmentItem>(
    ...
    async create(createAssignmentItemDto: CreateAssignmentItemDto): Promise<AssignmentItemResponseDto> {
        const { assignmentId, contentType, contentId, status } = createAssignmentItemDto;
